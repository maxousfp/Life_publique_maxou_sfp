...
waituntil {!isnil "fn_SaveToServer" && !isnil "sfp_config_liste_addons_autorise_anti_hack"};
...
[] spawn
{
	...
	waitUntil {sleep 0.5; !isnil "listes_joueurs_banni_automatiquement" && !isnil "sfp_config_activer_antihack_automatique_liste_ban"};
	if (getplayeruid player in listes_joueurs_banni_automatiquement && sfp_config_activer_antihack_automatique_liste_ban) then
	{
...
if (count detection_d_un_addon_cache >= 1 && sfp_config_activer_antihack_By_Pass_Addons) then
{
...
[] spawn 
{
	if (sfp_config_activer_antihack_Variables) then
	{	
		while {true} do
		{
...
[] spawn
{
	if (sfp_config_activer_antihack_Dialogues) then
	{	
